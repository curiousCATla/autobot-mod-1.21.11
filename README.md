...
- **Smooth Rotation** — Instantly snap or smoothly turn to cardinal directions; manual 15° yaw adjustments via keybinds
...
- **Scriptable Path Macros** — Write plain-text macro files to sequence any combination of moves, climbs, waits, and rotations; hot-swap between multiple scripts in-game
...
| Rotation | `RotationController.java` | 15° instant snaps; 4°/tick smooth facing |
| Movement | `MovementController.java` | Walk/strafe/climb with centering & stuck detection |
...
MOVE  <UP|DOWN|LEFT|RIGHT>  <blocks>  <true|false>
CLIMB <UP>                  <blocks>
WAIT  <ticks>
```
...
Steps execute sequentially; each step waits for the previous movement or rotation to finish before proceeding. The macro controller pauses automatically between steps (4-tick gap) to keep actions deterministic.
